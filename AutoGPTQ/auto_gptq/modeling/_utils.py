...
def make_quant(
    ...
    if use_low_high:
        from ..nn_modules.qlinear.low_high_simqlinear import QuantLinear
    else:
...
def pack_model(
    ...
    make_quant(model, quantizers, bits, group_size, use_triton=use_triton, use_cuda_fp16=use_cuda_fp16, desc_act=desc_act,use_low_high=use_low_high)
    ...
    for name in qlayers:
        ...
        if use_low_high:
            qlayers[name].pack(layers[name], scale, zero, g_idx,quantizers[name])
        else:
