...
from ..quantization.low_high_quantizer import LowHighQuantizer,Quantizer
    ...
    def quantize(
        ...
        for i in range(len(layers)):
            ...
            for names in inside_layer_modules:
                ...
                gptq = {}
                for name in subset:
                    if low_high_quant_config is not None:
                        quantizer=LowHighQuantizer(**low_high_quant_config)
                    else:
                        quantizer=Quantizer()
                        quantizer.configure(
                    ...
                    gptq[name] = GPTQ(subset[name],quantizer)
        ...
        pack_model(
            ...
            use_low_high=low_high_quant_config is not None
        )
        if device_map:
