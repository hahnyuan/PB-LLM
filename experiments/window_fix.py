...
def replace_qlinear(root_module, name_prefix=""):
    ...
    for name, module in module_name_dict.items():
        if isinstance(module, nn.Linear):
            ...
            if "outlier_window_fix" in args.binarization_method:
                qlinear = OutliersQLinearWindowFix(
                    module.weight,
                    module.bias,
                    dense_class=quant_cls,
                    outlier_fraction=args.outlier_fraction,
                )
            elif "act_outlier_column" in args.binarization_method:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--binarization_method",
        type=str,
        default="xnor_outlier_window_fix",
        choices=[
            "ste",
            "ir",
            "fda",
            ...
            "ste_outlier",
            "xnor_outlier",
            "xnor_outlier_window_fix",
        ],
    )
    parser.add_argument(
        "--outlier_fraction", type=float, default=0.125, help="Percentage of outliers"
    )
    parser.add_argument(
